...
describe('resource webhook', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.api.v1.sentry.webhook.create({
      action: 'action',
      actor: { id: 'string', name: 'name', type: 'user' },
      data: {},
      installation: { uuid: 'uuid' },
      'request-id': 'request-id',
      'sentry-hook-resource': 'sentry-hook-resource',
      'sentry-hook-signature': 'sentry-hook-signature',
      'sentry-hook-timestamp': 'sentry-hook-timestamp',
    });
  ...
  test.skip('test: required and optional params', async () => {
    const response = await client.api.v1.sentry.webhook.test({
      body: {},
      'x-sentry-signature': 'x-sentry-signature',
    });
  });
});
