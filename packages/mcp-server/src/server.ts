...
import { ClientOptions } from '@teardown/mcp';
...
export function initMcpServer(params: {
  ...
  const client = new Teardown(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client =
    params.client || new Teardown({ defaultHeaders: { 'X-Stainless-MCP': 'true' }, logger: logger });
